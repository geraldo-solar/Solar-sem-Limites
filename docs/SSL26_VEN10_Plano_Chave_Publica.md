...
1. **Motor, site público**: gravar reserva e hóspede por uma rota do servidor (já existe `api/create-reservation.ts`, usada pelo chatbot — passa a usar a chave do servidor e a servir também o site); **cancelamento e pré-check-in buscam só a reserva do código informado**, pelo servidor, em vez de baixar todas as reservas no navegador.
   **Feito em 23/09 (site público do Motor):** busca, criação, cancelamento, pré-check-in e cupom passam por `api/check-reservation` (o plano da Vercel limita a 12 rotas; a resposta antiga do GET continua igual). O visitante não carrega mais reservas nem cupons (antes baixava até 1.000 reservas com dados dos hóspedes e guardava no aparelho; a cópia antiga é apagada ao abrir o site). Busca só pelo número completo (link do e-mail) ou código de 8 caracteres + e-mail — antes qualquer pedaço do número achava a reserva de outra pessoa. Pré-check-in só completa ficha de mesmo CPF ou sem documento (antes bastava o nome). Conferido no ar e com reserva de teste (criada e cancelada; estoque voltou ao início). **Achado:** o banco já tira 1 do estoque quando a reserva entra; o site e a rota do chatbot tiravam de novo (2 por dia por reserva) — corrigido; datas futuras podem estar com menos disponibilidade que a real. **Falta:** Geraldo cadastrar `SUPABASE_SERVICE_ROLE_KEY` na Vercel do Motor (hoje as rotas usam a chave pública, que deixa de gravar na fase 3); o painel do Motor segue direto no banco com login.
