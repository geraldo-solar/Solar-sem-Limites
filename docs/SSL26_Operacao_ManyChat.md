...
**10h08 — publicação autorizada concluída:** ERP `50e234f`, origem `ee608f7` e site `4e041f1`, todos produção/READY. Instagram e WhatsApp publicados e reabertos, **STOPPED/Salvo e sem gatilho**, com os cinco pontos conferidos. Doze verificações HTTP passaram, inclusive 401 sem autenticação e 503 com processamento bloqueado. Sem contato, mensagem, configuração ou migração nova. Próximo: conferência de tarefas/evidências, cobertura das automações e piloto com autorização própria. [Publicação e limites](SSL26_Guardas_Publicacao_2026-09-24.md). **84% mantidos; AUT-06 parcial.** As notas abaixo preservam as etapas anteriores à publicação.

**Etapa seguinte, somente local:** servidor do ERP alinhado às seis tags dos rascunhos, incluindo `SSL26_PAUSA_REVISAO`. Rota privada para processar uma exclusão já registrada preparada, autenticada e desligada por padrão; sem agenda ou preenchimento retroativo. **738 testes ERP + 99 da página aprovados**, sem falhas/ignorados; TypeScript e ESLint aprovados. Sem commit, publicação, flags, contatos ou mensagens. Próximo: publicação coordenada autorizada, com controles desligados, seguida da conferência de tarefas/identidades e de piloto com escopo próprio. [Entrega e limites](SSL26_Processador_Exclusoes_Local_2026-09-24.md). **84% mantidos; AUT-06 parcial.**

**09h47 — cinco pontos dos rascunhos ajustados com autorização:** duas verificações do Instagram e três do WhatsApp receberam ausência de `SSL26_QA`, `SSL26_ATENDIMENTO_PAUSA` e `SSL26_PAUSA_REVISAO`, preservando condições anteriores. Todos relidos após recarregar. Instagram **DRAFT/Salvo**; WhatsApp **STOPPED/Salvo**, revisão ainda não publicada. Nenhum contato ou mensagem. Correção local adicional da releitura de identidade no e-mail, sem implantação; 99 testes aprovados. [Entrega e próxima sequência](SSL26_Guardas_Rascunhos_2026-09-24.md). AUT-06 parcial; **84% mantidos**.

**Filtros de exclusão salvos nos dois canais:** Brevo #16 (quatro atributos booleanos ligados por OU, salvo em 23/09) e ManyChat `SSL26 | Exclusões - não enviar` (seis tags ligadas por OU, salvo e reaberto em 24/09). Nenhum contato alterado, mensagem enviada ou campanha ativada. **Ainda falta integrar as condições completas aos pontos reais de envio e concluir a conciliação/homologação; filtros salvos não se aplicam automaticamente aos fluxos.** [Configuração e evidências](SSL26_Filtros_Exclusao_2026-09-24.md). Preparação mantida em **84%**, AUT-06 parcial.

As duas tags e os três atributos Brevo antes pendentes foram instalados em 23/09, sem aplicação a contatos nessa etapa: [metadados](SSL26_Metadados_Canais_2026-09-23.md). As notas abaixo preservam o histórico, inclusive percentuais e pendências já superados; consultar o checklist para o estado executivo atual.

Versão histórica de preparação — 21/09/2026. **Naquela conferência: 14 tags e 13 campos instalados. Instagram continuava em rascunho; boas-vindas WhatsApp convertido para modelo aprovado e publicado sem gatilho público. Fluxo de atendimento publicado sem gatilho, destinado provisoriamente a Geraldo e testado pontualmente; demais fluxos ainda não instalados.** Testes autorizados são específicos e não liberam campanha ou reinscrição. Preserva preço, datas e regras já aprovados.

### Histórico — atendimento em 21/09/2026

**Aprovação PCDA — revisão local posterior:** decisão com senha/permissão do funcionário, auditoria, transação e proteção de repetição preparadas/testadas, incluindo vínculo antigo sem novo e-mail. **538 testes ERP + 69 página**, além de interface desktop/mobile. Migração/rotas não publicadas; controles de comprador continuam desligados e não conectados ao checkout. Contatos/fluxos reais preservados; 79% mantidos. [Entrega e próxima sequência](<../../ERP Hotel Solar/docs/pcda-aprovacao-autenticada.md>).

**Comprador — nova preparação local:** adaptador agora aceita `SSL26_COMPRADOR` a partir de fila produzida por conferência privada de pedido/campanha/lead. Contrato de origem e conferência foram testados, mas ainda não estão conectados ao checkout. Tag já existente no ManyChat; atributo Brevo não criado. Fluxos e contas reais preservados. 497 testes ERP + 69 da página aprovados. [Condições e pendências](<../../ERP Hotel Solar/docs/ssl26-comprador-campanha.md>).

**Preparação local posterior à auditoria:** registro central e propagação de QA/atendimento implementados, não publicados. O manifesto local prevê duas novas tags (`SSL26_QA`, `SSL26_ATENDIMENTO_PAUSA`), totalizando 16 propostas/13 campos; **produção continua na última posição conferida de 14 tags/13 campos**, sem instalação nesta rodada. Fluxos visuais e contatos não alterados. Retenção da campanha não se remove automaticamente ao fechar atendimento; compra ainda depende de atribuição confiável. 476 testes ERP + 67 da página aprovados. [Limites, migração e próxima etapa](<../../ERP Hotel Solar/docs/ssl26-exclusoes-audiencia.md>).

**13h57 — audiência por canal conferida:** dois contatos `SSL26_LEAD`, ambos já vinculados ao ERP e descadastrados. Filtro temporário com ausência de `SSL26_OPT_OUT` retornou zero; nenhum segmento salvo, tag alterada ou ação em lote. Brevo #24 vazio; seis atributos SSL26 existentes não incluem controles específicos de QA/atendimento/compra. Verificador local preparado, 29 testes novos; não é uma exclusão instalada na campanha. **79% mantidos; AUT-06 parcial.** [Auditoria atual e próximos controles](SSL26_Audiencia_2026-09-21.md).

**13h45 — falha controlada e recuperação validadas após novo aceite.** Cabeçalho inválido da cópia salvo/publicado e relido após recarregamento; uma execução manteve `SSL26_PAUSA_REVISAO` e confirmação falsa. Segunda e última execução, no núcleo normal preservado, confirmou pausa e retirou somente a pendência. Saída, demais tags/campos/listas/entregas preservados; zero evento central novo e sem mensagem ao cliente. Cópia **Service stopped**, sem gatilho e com restrição exclusiva; recibo encerrado bloqueia novas chamadas. **79% mantidos; AUT-05 parcial por timeout real e conversa inicialmente fechada/com outro responsável.** Próxima frente: audiência/exclusões (AUT-06), sem envios. [Provas e limites](SSL26_Teste_Falha_Recuperacao_2026-09-21.md). A rodada inconclusiva abaixo é histórica.

**13h22–13h25 — ensaio de falha inconclusivo:** cópia `SSL26_QA_401_GERALDO_20260921_SEM_GATILHO`, ID `content20260921130834_412814`, salva com condição inicial exclusiva do titular e sem gatilho. Uma execução autorizada percorreu confirmação **yes→no→yes**, retirando a pendência: a alteração para autenticação inválida não persistiu no editor. Verificação do erro falhou corretamente; sem segunda execução ou repetição. Saída, pausa e demais dados preservados em leitura; cópia **Service stopped**, uma execução. Núcleo original e credencial preservados. **79% mantidos; AUT-05 parcial; nova tentativa exige novo aceite.** [Provas, diagnóstico e limites](SSL26_Teste_Falha_Atendimento_2026-09-21.md). As notas de 20/09 abaixo são históricas.

**22h48 — entrada real validada e desligada:** frase recebida às 22h36, uma execução da entrada e incremento do núcleo de 2 para 3. Histórico comprova inclusão da pendência, confirmação yes→no→yes e retirada da pendência. Conversa aberta e responsável `Eu`, perfil conferido como Geraldo. Comparação de leitura com a linha de base preservou saída/pausa em ManyChat/ERP/Brevo, demais campos/listas/tags e entregas, sem evento central novo (já estava pausado). Gatilho Off/STOPPED relido às 22h38; restrição de QA mantida. Sem resposta automática. **77% mantidos; AUT-05 parcial por falha/timeout e conversa fechada ainda pendentes.** [Resultado e limites](SSL26_Entrada_Atendimento_2026-09-20.md). Autorização única utilizada; não reativar/repetir sem novo aceite.

**22h36 — entrada instalada e teste restrito autorizado:** conexão recuperada; criado `SSL26_04_ENTRADA_ATENDIMENTO_WA`, ID `content20260921012529_041356`, com duas etapas sem mensagens, chamando o núcleo existente. Primeiro salvo sem publicação e com palavra-chave desligada. Após novo aceite de Geraldo, instalada/relida condição conjunta por WhatsApp preenchido e ID nativo do próprio contato final 1312; publicada/ativada temporariamente. Demais contatos encerram sem ações. Aguarda frase iniciada pelo titular, verificação e desligamento posterior. Campo de confirmação relido normalmente no núcleo. **Campanha desligada; 77% mantidos; AUT-05 parcial.** [Configuração, autoridade e provas](SSL26_Entrada_Atendimento_2026-09-20.md). As notas seguintes preservam o histórico.

**22h03 — preparação da entrada:** definida a entrada exclusiva WhatsApp por correspondência exata de `atendimento solar sem limites`, chamando o fluxo de ações já publicado, sem mensagens nem filtros de aquisição. **É especificação local, ainda não salva/ativada no ManyChat:** o controle do navegador falhou antes de qualquer edição. A API somente leitura confirmou os 14 nomes de tags e 13 campos, incluindo o booleano de confirmação; 16 testes locais do receptor passaram. [Entrada, ensaio e bloqueios](SSL26_Entrada_Atendimento_2026-09-20.md). **AUT-05 parcial; percentual geral vigente: 77%.** As notas percentuais anteriores são históricas.
...
> A visita guiada será em 24/11/2026, às 19h. As vendas abrem em 25/11, às 8h, e encerram em 01/12, às 23h59. Horários de Belém. Por enquanto, você pode acompanhar o Canal VIP.

Revisão editorial de 20/09: fechamento somente por data, conforme o checklist vigente. Não anunciar esgotamento em 200 pacotes, vagas restantes ou escassez de quantidade. Esta correção é documental; o FAQ ainda não está instalado como automação pública.
...
Primeira entrada homologada no QA restrito: frase exata `atendimento solar sem limites` no WhatsApp, sem substituir a resposta padrão do hotel. Testada uma vez pelo titular; **gatilho desligado e restrição por ID mantida**. Resultado e cenários ainda pendentes no [roteiro de entrada](SSL26_Entrada_Atendimento_2026-09-20.md). Um futuro botão **Falar com equipe** pode usar o mesmo destino, mas não será acrescentado aos três botões do modelo de boas-vindas aprovado. Instagram e menu completo de dúvidas continuam separados e pendentes.
